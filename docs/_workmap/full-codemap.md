...
> **진행 상태 (2026-06-01)**: Step 1~6 **코드 완료 + 커밋됨**(테스트 215 green).
> Step 7(50~100 iter GPU run)은 operator. proposal/codemap 대비 **MVP 범위**:
> override precedence=MVP(opportunity/cooldown nudge 미구현), combine=distinct-family
> MVP(diff-overlap 검사 후속), cooldown=**soft only**(hard gate 는 첫 run 오탐 확인 후
> §8), repair_event=verify_fail MVP, prompt=single-call(2-call §5.2 후속). 남은 후속:
> hard gate, legacy `_iteration_mode`/`_EXPLOIT_DIRECTIVE` cleanup. 본 폴더는 후속까지
> 끝나면 삭제.
>
> proposal `2026-06-01-from-scratch-discovery-harness.md` 전체 구현 지도. 본 문서는
> **Step 2~7**. `docs/_workmap/` 는 임시.
