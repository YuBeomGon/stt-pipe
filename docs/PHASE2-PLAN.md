...
본 인프라는 **잡 전체 (25 iter) 종료 후, 루프 진행 자체를 검증** 하기 위한 도구다.
"최종 corpus_cer 가 좋아졌나?" 만 보면 *어떻게 좋아졌는지·어디서 깨졌는지* 를 영영
모르기 때문에 다음 질문에 답하는 도구를 미리 만든다:

- 진화가 **한쪽 방향으로만 쏠렸는가** (예: chunking 만 25 회 손대고 prompt·decode 는 안 만짐 → 탐색 다양성 결함)
- 큰 개선이 **1~2 iter 집중** (운 좋은 한 방) 인가 **점진 누적** (시스템적 개선) 인가
- 가드가 **정상 가설도 같이 죽이지 않았는가** (가드 임계 과도)
