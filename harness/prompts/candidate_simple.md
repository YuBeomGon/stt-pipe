...
> `harness.prompt_simple.build_simple_prompt`. The runtime prompt also supplies
> the current state, parent/inspirations, and a findings ledger — this profile
> defines the *role, the surface you must discover, the reconnaissance
> checklist, and the required output*. Edit this file to change candidate
> behavior; the next iteration picks it up.
...
your code change, then evaluates. If verification passes and the result
improves on the best, `best.txt` advances; either way your candidate and its
`what_i_learned` are kept forever in the flat archive.
